...
2. **Reduce human loops with agent.** Engineers can spend more time on task that are Important and urgent whilst the Important but low-urgency tasks can be delegated to the harness.

## How this compares to Ralph loops

[Geoffrey Huntley's Ralph loop](https://devinterrupted.substack.com/p/inventing-the-ralph-wiggum-loop-creator): a bash while-loop that feeds the same prompt to a coding agent until done. Filesystem is the memory; conversation history isn't. Same context every iteration, different codebase on disk. The pattern works because LLMs degrade as context fills past a certain threshold (Huntley's framing: minimize allocation to avoid the "Dumb Zone" of 60-70% context fill).
...
The buildkit harness isn't competing with either. It contains two skills, a shared parser, ~600-LOC state-machine core (~830 LOC total), intentionally hand-readable and opinionated. Use superpowers or Matt's if you want a full kit. Take mine if you want a simple scaffold to fork.

## Using AskUserQuestion Tool

The architect's interviews uses Claude Code's `AskUserQuestion`. Anthropic backs the pattern in their [How We Claude Code workshop](https://github.com/anthropics/cwc-workshops/tree/main/how-we-claude-code) and the [video walkthrough](https://www.youtube.com/watch?v=IlqJqcl8ONE). The workshop's three phases map onto my harness:

- **Phase 1 (interview brainstorm)** ≈ architect interviews via `AskUserQuestion`
...
The interview pattern isn't mine. [Matt Pocock's `grill-with-docs`](https://github.com/mattpocock/skills/tree/main/skills/engineering/grill-with-docs) is a Socratic interview that stress-tests a plan against the project's domain language and maintains a `CONTEXT.md` glossary plus ADRs. I applied the interview shape one layer deeper: my version produces a structured contract per GitHub issue and adds a rule that the unblock command must cite real human evidence (a PR comment ID or a git commit SHA) before the loop can resume. The framing is Matt's. 
...
Judgement: "when to escalate to a human", "how to triage a blocked pipeline or intervene" which often knowledge based.
...
I added four hard caps: per-run dollar budget, per-agent wall-clock timeout, per-proof timeout, max review rounds. These are still work in progress. The auditor runs with a tighter tool permission set: read-only, so it can't mutate code.

The dollar budget was shared across builder and auditor, which isn't the best design since the auditor is read-only and should run on a quarter of the budget. This is flagged on the redo list.

I had no notification system so the operator only finds out at the cap. A "round 2 of 3" notification/escalation would let me intervene early to better understand anomaly events.

## XML parsing

I swapped some markdown headings to XML tags so the harness could inject one slice per agent. The slicing happens before each `claude -p` spawn: the build-loop reads `context.md` (the per-issue contract), pulls each `<tag>` block out with a small regex, and re-assembles a role-specific prompt. The shared parser lives in `skills/lib/context.mjs`.
...
- **`proof_command`**: the single shell command the harness runs to verify "done" (e.g., `npm test`, `cargo check`, `bun run e2e`). This is the ground-truth gate.
...
Two roles with two slices and one source document. XML makes this a tiny regex. Markdown headings would need fragile boundary detection (where does the section end? next `##`? indented child heading?).

Anthropic's guide says XML helps the model parse unambiguously, but I haven't been able to benchmark whether `<tag>` beats `## heading` at the model level. The parser win is obvious in this case.

I call this role the **auditor**, not a reviewer. It doesn't see the diff and doesn't mutate, just outputs a binary verdict as a compliance check, not a full code review. "Reviewer" is the human at the HITL gate (who reads the diff and posts a citable comment).

## HITL gates only work if the human actually loops in
...
**Postscript (June 15, 2026).** Anthropic [split off `claude -p`, the Agent SDK, and Claude Code GitHub Actions onto a separate monthly credit pool](https://www.xda-developers.com/anthropics-claude-subscriptions-no-longer-include-agent-sdk-and-claude-p-usage/) from the regular Pro/Max/Team subscription. This means interactive Claude Code in the terminal still draws from the regular subscription. For a buildkit-style harness that spawns `claude -p` per round, this soon changes the billing surface. Usage will meter against the Agent SDK credit pool instead of the subscription quota. The `ANTHROPIC_API_KEY` strip still matters (you don't want a surprise API-key bill on top of the credit pool)
...
- **Use `--system-prompt-file` for prompt caching.** The cache rewards stable prefixes. My current code rebuilds the prompt inline every round and doesnt gets a cache hit.
- **Tighten the auditor's budget below the builder's.** Read-only audits don't need the builder's headroom.
...
- **Use a lighter coding agent.** Pi's minimal system and extensibility lets you do actual context engineering: control what goes into the context window and how it's managed. [nicobailon/pi-subagents](https://github.com/nicobailon/pi-subagents) is the practical primitive: parent Pi session delegates to focused child agents.

## Key takeaways

A harness spends money instead of time. If your time is worth more than a few dollars per useful change and the issue is well-scoped, the trade works. Otherwise if the tasks is ambiguous, open the PR yourself.

Don't treat the buildkit harness as production infrastructure. Treat it as a POC substrate you'll refactor per project. What I built is ~600 lines of state machine, opinionated. Expect to rewrite half to your taste.

If you want to give it a try or found smelly code you want to help improve. I added a CONTRIBUTING.md you can read.
